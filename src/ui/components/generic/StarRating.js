...
const Star = ({onClick, rated, value, hover, onMouseEnter, onMouseLeave}) => {
    ...
    return (<span className={"star p-1" + (rated || hover ? " selected": "")} onClick={() => onClick(value)} onMouseEnter={() => onMouseEnter(value)} onMouseLeave={() => onMouseLeave(value)}>
...
const StarRating = ({rated=0, disabled=false, onSelect}) => {
    ...
    return (
        <div className="d-inline-flex" onMouseLeave={() => setHover(0)}>
            {
