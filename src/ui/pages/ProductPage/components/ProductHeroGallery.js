...
function ProductHeroGallery({images, productName }) {
    ...
    return (
        <div className="product-thumb-gallery col-lg-6 d-md-flex flex-md-nowrap float-left position-relative">
            ...
            <div className="product-image-thumbnails no-scroll-bar overflow-auto clearfix col-12 col-md-2 d-flex flex-md-column justify-content-start float-left p-2">
                {
            ...
            <div className="product-zoomed-image col-md-10 float-left p-0">
                    {
                        (images || []).map((item, index) => (
                            <AppImage key={index} className={"mb-2 w-100" + (selected === index ? " selected" : " position-absolute")} src={(decodeURI(item) || "").trim()} alt={productName} title={productName}/>
                        ))
                    }
                    <AppImage className="mb-2 w-100" src={(decodeURI(images[0]) || "").trim()} alt={productName} aria-label={productName}/>
            </div>
