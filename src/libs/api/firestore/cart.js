...
export const getUserCart = async (email) => {
    ...
    try {
        ...
        window.logerror('getUserCart:Error ', err);
...
export const updateUserCart = async ({products, email}) => {
    ...
    try {
        ...
        */
        ...
        window.loginfo('api:firestore:updateUserCart::response', update);
        return new Promise((resolve, reject) => resolve(cart));

    }catch(error){
        window.logerror('getUserCart:Error ', error);
        //return new Promise((resolve, reject) => reject(err));
...
export const addProductToCart = async (email, product, variant=null) => {
    ...
    try {
        ...
        window.loginfo('Products after adding -> ', present, products);
        cart = Cart({id, products});

        let update = await updateUserByEmail(email, {cart});
        window.loginfo('api:firestore:addProductToCart::response', update);
        return new Promise((resolve, reject) => resolve(cart));
        
    }catch(error){
        window.logerror('api:firestore:addProductToCart::response', error);
        return new Promise((resolve, reject) => reject(error));
    }
