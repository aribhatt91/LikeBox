...
const applyFilter = (query, filterObject) => {
    ...
    window.loginfo('applyFilter', query, filterObject);
...
export const fetchFirestoreProducts = async (queryPaths=[], page=0, LIMIT=10, LAST_NODES=[], filterObject=null) => {
    //window.loginfo('Product category ->', queryPaths);
    let res = null;
    try {
        ...
        if(queryPaths.length > 1){
            ...
            //window.loginfo('2: Fetched Firestore products for path -> ', queryPaths.join('-'), '  --->', res);
            ...
            //window.loginfo('1: Fetched Firestore products for path -> ', queryPaths[0], '--->', res);
        }
    }catch(err){
        window.logerror('fetchFirestoreProducts:error', err);
...
export const fetchFirestoreProduct = async (sku) => {
    ...
    try {
        ...
        res = doc.data();
        window.loginfo('fetchFirestoreProduct:response', res);
    }catch(err){
        window.logerror('fetchFirestoreProduct:error', err);
        return new Promise((resolve, reject) => reject(err));
    }
    return new Promise(resolve => resolve(res));
}
...
export const fetchProductBrands = async (categories) => {
    ...
    try {
        ...
        window.loginfo('fetchProductBrands:start:', category);
        if(category){
            try{
        ...
        window.loginfo('fetchProductBrands:res:', res);
        return new Promise(resolve => resolve(res));
    }catch(err){
        window.logerror('fetchProductBrands', err);
    }
