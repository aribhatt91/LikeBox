...
let CartService = {
    ...
    addToCart: (email, product, variant=null, notify=null) => {
        ...
        return dispatch => {
            ...
            if(!CartService.validateProduct(product)){
                window.loginfo('addToCart: ', INVALID_PRODUCT, product);
                //DISPATCH ERROR
            ...
            addProductToCart(email, product, variant).then( res => {
                ...
                //window.loginfo('addToCart: MockGetCart: parsed cart', CartService.active_cart);
                ...
                //window.loginfo('addToCart: MockGetCart: error', error);
                dispatch(cartError({error: CART_ADD_ERROR, cart: CartService.active_cart}));
            });
        }
    ...
    removeFromCart: (email, product, decrement=false, notify) => {
        return (dispatch) => {
            dispatch(updateCartPending(CartService.active_cart));
            if(!CartService.validateProduct(product)){
                window.loginfo('removeFromCart: ', INVALID_PRODUCT, product);
                //DISPATCH ERROR
                if(decrement){
           ...
           removeProductFromCart(email, product, decrement).then( res => {
                CartService.setCart(CartService.parseCart(res));
                window.loginfo('removeFromCart: removeProductFromCart: parsed cart', CartService.active_cart);
                ...
                window.loginfo('removeFromCart: removeProductFromCart: error', error);
                dispatch(cartError({error: CART_REMOVE_ERROR, cart: CartService.active_cart}));
            })
    ...
    fetchCart: (email) => {
        window.loginfo('fetchCart: email -> ', email);
        return (dispatch) => {
            dispatch(fetchCartPending(EMPTY_CART));            
            getUserCart(email).then(res => {
                CartService.setCart(CartService.parseCart(res));
                window.loginfo('getUserCart: parsed cart', CartService.active_cart);
            ...
            .catch(err => {
                window.loginfo('fetchCart: getUserCart: error', err);
                dispatch(cartError({error: CART_FETCH_ERROR, cart: CartService.active_cart}));
            })
        }  
    },
    clearCart: (email) => {
        window.loginfo('clearCart: email -> ', email);
        return (dispatch) => {
            dispatch(fetchCartPending(EMPTY_CART));            
            updateUserCart({email}).then(res => {
                CartService.setCart(CartService.parseCart(res));
                window.loginfo('clearCart: parsed cart', CartService.active_cart);
                dispatch(fetchCartSuccess(CartService.active_cart));
            })
            .catch(err => {
                window.logerror('clearCart: getUserCart: error', err);
                dispatch(cartError({error: CART_FETCH_ERROR, cart: CartService.active_cart}));
            })
        }  
    },
    parseCart: (cartString) => {
        try{
            ...
            window.loginfo('parseCart Error', e);
